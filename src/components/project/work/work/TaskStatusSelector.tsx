...
function TaskStatusSelector() {
    ...
    function onChangeHandler() {
        setProgressStatus((prev) => {
            if(prev.progressStatusCode === TASK_STATUS.PS003.value) {
                return {
                    progressStatus: TASK_STATUS.PS002.name,
                    progressStatusCode: TASK_STATUS.PS002.value
                }
            }else{
                return {
                    progressStatus: TASK_STATUS.PS003.name,
                    progressStatusCode: TASK_STATUS.PS003.value
                }
            }
    ...
    return (
        <Switch
            checked={progressStatusCode === TASK_STATUS.PS003.value}
        ...
        >
            <span className="sr-only">{progressStatus}</span>
            <span
