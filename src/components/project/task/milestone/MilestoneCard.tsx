...
import {milestoneStateStore} from "@/store/project/task/MilestoneStateStore";
...
function MilestoneCard({milestoneInfo}: MilestoneCardProps) {
    const [{activeId}, setMilestone] = useRecoilState(milestoneStateStore);
    ...
    function onClickContentHandler(e: Event) {
        e.preventDefault();
    ...
    return (
        <div
            className={`relative flex pc:max-w-[300px] tablet:max-w-[180px] items-center justify-between truncate rounded-md border border-gray-200 bg-white overflow-visible ${activeClass}`}>
            <div className="flex-1 truncate px-4 py-2 text-sm">
                <a
                    href='javascript;'
                    className={`pc:text-xl tablet:text-lg ${textClass} font-medium hover:text-secondary`}
