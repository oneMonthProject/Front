...
const PostListSkeleton = ({ itemCount = 8 }: { itemCount: number }) => {
  ...
  return (
    <div className="mt-6 mobile:mt-2">
      <div className="grid justify-items-center pc:grid-cols-4 tablet:grid-cols-2 mobile:grid-cols-1 gap-10 mobile:gap-5">
        {
          items.length > 0 && items.map((_, idx) => (
            <div key={idx} className="flex-col w-[280px] rounded-xl mobile:w-full mobile:mt-2">
