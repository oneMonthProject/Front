...
export async function updateProjectInfo(projectInfo: ProjectInfoUpdateReq) {
    ...
    throwErrorIfInvalid(!milestoneAuth, '프로젝트 수정 권한이 없습니다.');
    throwErrorIfInvalid(_.isEmpty(projectName), '프로젝트 이름을 입력해주세요');
    throwErrorIfInvalid(_.isEmpty(subject), '프로젝트 주제를 입력해주세요');
    throwErrorIfInvalid(_.isNull(trustGradeId), '프로젝트 신뢰등급을 선택해주세요');
    throwErrorIfInvalid(_.isEmpty(startDate), '시작날짜를 선택해주세요');
    throwErrorIfInvalid(_.isEmpty(endDate), '종료날짜를 선택해주세요');
